...
Shader "Custom/CookTorranceSurface" {
    Properties {
        ...
        metallic ("metallic", Range(0,1)) = 0.5
    ...
    SubShader {
                                        ...
                                        , float LdotH, float roughness)
        {
            ...
            float d = lightScatter * viewScatter * energyFactor;
            ...
            return lerp(d, ss, _Subsurface);
        }

        //Cook-Torrance 
        inline float3 CookTorranceSpec(float NdotL, float LdotH, float NdotH, float NdotV, float roughness, float F0,float metallic){
            ...
            F = lerp(F0 + (1.0 - F0) * LdotH5,1,metallic);
        ...
        inline float4 LightingCookTorrance (SurfaceOutputCustom s, float3 viewDir, UnityGI gi){
            ...
            float3 diff = DisneyDiff(s.Albedo, NdotL,  NdotV, LdotH, _Roughness) * (1-metallic);
            float3 spec = CookTorranceSpec(NdotL, LdotH, NdotH, NdotV, _Roughness, _SpecColor.g,metallic);
            float3 diff2 = (DisneyFrostbiteDiff(NdotL, NdotV, LdotH, _Roughness) * s.Albedo)/PI * (1-metallic);
